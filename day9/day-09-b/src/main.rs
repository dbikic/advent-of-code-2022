...
fn main() {
    ...
    let knots_size = 10;
    for _ in 0..knots_size {
        knots.push((1000, 1000));
    }
    ...
    include_str!("../input.txt")
        ...
        .for_each(|x| {
            ...
            for _ in 0..step.distance {
                ...
                for i in 0..knots_size {
                    let referent_knot = if i == 0 {
                        &head
                    } else {
                        &knots[i - 1]
                    };
                    if !are_touching(&referent_knot, &knots[i]) {
                        let tail_move = get_tail_movement(&referent_knot, &knots[i]);
                        knots[i].0 += tail_move.0;
                        knots[i].1 += tail_move.1;
                    }
                    if i == knots_size - 1 {
                        tail_positions.insert(knots[i]);
                    }
                }
            }
        });
